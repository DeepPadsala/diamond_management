...
class DiamondPartyInvoice(models.Model):
    """Monthly invoice to a party for labour on their diamond packets."""
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if not vals.get("name") or vals["name"] == _("New"):
                vals["name"] = self.env["ir.sequence"].with_company(company).next_by_code("diamond.party.invoice") or _("New")
    ...
    def action_cancel(self):
        for rec in self:
            rec.entry_ids.write({"state": "draft", "party_invoice_id": False})
            rec.line_ids.unlink()
            rec.state = "cancelled"
...
class DiamondPartyInvoiceLine(models.Model):
    _name = "diamond.party.invoice.line"
    _description = "Party Invoice Line"
    _order = "id"

    invoice_id = fields.Many2one("diamond.party.invoice", required=True, ondelete="cascade")
